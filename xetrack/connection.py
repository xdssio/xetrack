...
class DuckDBConnection:
    def __init__(self, db: str = "track.db", compress: bool = False):
        """
        ...
        self.conn: duckdb.DuckDBPyConnection = self._init_connection()
    ...
    def _init_connection(self):
        ...
        is_attached = conn.execute(
            f"SELECT * FROM pragma_database_list WHERE name = '{db}'"
        ).fetchall()
        if not is_attached:
    ...
    @property
    def columns(self):
        return set(self.dtypes.keys())  # type: ignore
